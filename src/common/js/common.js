...
const Common ={
  ...
  requestUrl : function(name){     
    ...
    var url = config.hosts + config.version + '/' +name + '?appid='+config.appid + '&channel='+ config.channel + '&time='+Util.formatDate.timestamp() + '&deviceid='+config.deviceid + '&apptype='+config.appType+'&hash='+hash;
    console.log("当前的url:" + url);
    console.log("当前的appkey:" + config.appkey);
    console.log("当前的appType:" + config.appType);
    console.log("当前的hash源码:" + config.appkey + config.version + name + config.appid + config.deviceid + config.channel + Util.formatDate.timestamp() );
    console.log("当前的hash:" + hash);
